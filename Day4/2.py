...
for num in bingo_nums[bingo_board_size-1:]:
    ...
    board_index = 0
    for board in list(bingo_board_solutions):
        if(check_bingo_board(board, taken_nums)):
            if(len(bingo_boards) <= 1):
                losing_board = bingo_boards[0]
            del(bingo_boards[board_index])
            del(bingo_board_solutions[board_index])
            continue
        board_index += 1
    if(losing_board):
        break

unmarked_numbers = [num for num in losing_board if num not in taken_nums]
