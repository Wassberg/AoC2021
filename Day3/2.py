...
def getNumbers(num_list, num, col):
    new_list = []
...
while(len(oxygen_gr_nums) > 1):
    ...
    oxygen_gr_nums = getNumbers(oxygen_gr_nums, toKeep, col_index)
    oxygen_gr_arr = getArr(oxygen_gr_nums)
    col_index += 1
...
while(len(co2_sr_nums) > 1):
    ...
    toKeep = '1' if ones < zeroes else '0'
    co2_sr_nums = getNumbers(co2_sr_nums, toKeep, col_index)
    co2_sr_arr = getArr(co2_sr_nums)
    col_index += 1
